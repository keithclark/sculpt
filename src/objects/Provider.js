...
export default class Provider {
  ...
  /**
   * Retreive a single item from a data store. The result should be a key/value.
   * list The Model this provider is connected to will use these values when
   * creating a new instance to populate its properties.
   *
   * @param {Object} [filters] - The bindings to filter against
   * @param {BindingMap} [bindings] - The bindings for the model
   * @returns {Promise} - A promise that resolves with data
   */
  async findOne(filters, bindings) {
    notImplemented('find');
  }
   ...
   * @param {BindingMap} [bindings] - The bindings for the model
   * @returns {Promise} - A promise that resolves with true if the update was made.
   */
  async update(filters, boundValues, bindings) {
   ...
   *
   * @param {Object} [filters] - The bindings to filter against
   * @returns {Promise} - A promise that resolves with true if the delete was completed.
   */
  async delete(filters, bindings) {
